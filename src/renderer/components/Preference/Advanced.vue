<template>
  <el-container class="content panel" direction="vertical">
    ...
    <el-main class="panel-content">
      <el-form class="form-preference" ref="advancedForm" label-position="right" size="mini" :model="form" :rules="rules">
          ...
          <el-col class="form-item-sub" :span="24">
            ...
            <div class="el-form-item__info" style="margin-top: 8px" v-if="form.lastCheckUpdateTime !== 0">
              {{
                ...
                ': ' +
                (form.lastCheckUpdateTime !== 0 ? new Date(form.lastCheckUpdateTime).toLocaleString() : new Date().toLocaleString())
              }}
        ...
        <el-form-item :label="`${$t('preferences.proxy')}: `" :label-width="formLabelWidth">
          <el-switch v-model="form.useProxy" :active-text="$t('preferences.use-proxy')" @change="onUseProxyChange"></el-switch>
        </el-form-item>
        <el-form-item :label-width="formLabelWidth" v-if="form.useProxy" style="margin-top: -16px">
          <el-col class="form-item-sub" :xs="24" :sm="20" :md="16" :lg="16">
            <el-input placeholder="[http://][USER:PASSWORD@]HOST[:PORT]" @change="onAllProxyBackupChange" v-model="form.allProxyBackup"></el-input>
          </el-col>
          <el-col class="form-item-sub" :xs="24" :sm="24" :md="20" :lg="20">
            <el-input type="textarea" rows="2" auto-complete="off" :placeholder="`${$t('preferences.no-proxy-input-tips')}`" v-model="form.noProxy"></el-input>
        ...
        <el-form-item :label="`${$t('preferences.bt-tracker')}: `" :label-width="formLabelWidth">
          <div class="form-item-sub bt-tracker">
            <el-row :gutter="10" style="line-height: 0">
              <el-col :span="20">
                <div class="track-source">
                  <el-select class="select-track-source" v-model="form.trackerSource" allow-create filterable multiple>
                    <el-option-group v-for="group in trackerSourceOptions" :key="group.label" :label="group.label">
                      <el-option v-for="item in group.options" :key="item.value" :label="item.label" :value="item.value">
                        <span style="float: left">{{ item.label }}</span>
                        <span style="float: right; margin-right: 24px">
                          <el-tag type="success" size="mini" v-if="item.cdn">CDN</el-tag>
                        </span>
                      </el-option>
                    </el-option-group>
                  </el-select>
              ...
              <el-col :span="3">
                <div class="sync-tracker">
                  <el-tooltip class="item" effect="dark" :content="$t('preferences.sync-tracker-tips')" placement="bottom">
                    <el-button @click="syncTrackerFromSource" class="sync-tracker-btn">
                      <mo-icon name="refresh" width="12" height="12" :spin="true" v-if="trackerSyncing" />
            ...
            <el-input type="textarea" rows="3" auto-complete="off" :placeholder="`${$t('preferences.bt-tracker-input-tips')}`" v-model="form.btTracker"></el-input>
            <div class="el-form-item__info" style="margin-top: 8px">
        ...
        <el-form-item :label="`${$t('preferences.rpc')}: `" :label-width="formLabelWidth">
          <el-row style="margin-bottom: 8px">
            <el-col class="form-item-sub" :xs="24" :sm="18" :md="10" :lg="10">
              {{ $t('preferences.rpc-listen-port') }}
              <el-input :placeholder="rpcDefaultPort" :maxlength="8" v-model="form.rpcListenPort" @change="onRpcListenPortChange">
          ...
          <el-row style="margin-bottom: 8px">
            <el-col class="form-item-sub" :xs="24" :sm="18" :md="18" :lg="18">
              ...
              <el-input :show-password="hideRpcSecret" placeholder="RPC Secret" :maxlength="24" v-model="form.rpcSecret">
        ...
        <el-form-item :label="`${$t('preferences.port')}: `" :label-width="formLabelWidth">
          <el-row style="margin-bottom: 8px">
            <el-col class="form-item-sub" :xs="24" :sm="18" :md="12" :lg="12">
              <el-switch v-model="form.enableUpnp" active-text="UPnP/NAT-PMP"></el-switch>
            </el-col>
          </el-row>
          <el-row style="margin-bottom: 8px">
            <el-col class="form-item-sub" :xs="24" :sm="18" :md="10" :lg="10">
              {{ $t('preferences.bt-port') }}
              <el-input placeholder="BT Port" :maxlength="8" v-model="form.listenPort">
          ...
          <el-row>
            <el-col class="form-item-sub" :xs="24" :sm="18" :md="10" :lg="10">
              ...
              <el-input placeholder="DHT Port" :maxlength="8" v-model="form.dhtListenPort">
        ...
        <el-form-item :label="`${$t('preferences.download-protocol')}: `" :label-width="formLabelWidth">
          ...
          <el-col class="form-item-sub" :span="24">
            <el-switch v-model="form.protocols.magnet" :active-text="$t('preferences.protocols-magnet')" @change="val => onProtocolsChange('magnet', val)"></el-switch>
          </el-col>
          <el-col class="form-item-sub" :span="24">
            <el-switch v-model="form.protocols.thunder" :active-text="$t('preferences.protocols-thunder')" @change="val => onProtocolsChange('thunder', val)"></el-switch>
          </el-col>
        </el-form-item>
        <el-form-item :label="`${$t('preferences.user-agent')}: `" :label-width="formLabelWidth">
          <el-col class="form-item-sub" :span="24">
            {{ $t('preferences.mock-user-agent') }}
            <el-input type="textarea" rows="2" auto-complete="off" placeholder="User-Agent" v-model="form.userAgent"></el-input>
        ...
        <el-form-item :label="`${$t('preferences.developer')}: `" :label-width="formLabelWidth">
          ...
          <el-col class="form-item-sub" :span="24">
            ...
            <el-input placeholder="" disabled v-model="logPath">
              <mo-show-in-folder slot="append" v-if="isRenderer" :path="logPath" />
            </el-input>
          </el-col>
          <el-col class="form-item-sub" :span="24">
            {{ $t('preferences.download-session-path') }}
            <el-input placeholder="" disabled v-model="sessionPath">
              <mo-show-in-folder slot="append" v-if="isRenderer" :path="sessionPath" />
            </el-input>
          </el-col>
          <el-col class="form-item-sub" :span="24">
      ...
      <div class="form-actions">
        <el-button type="primary" @click="submitForm('advancedForm')">
          {{ $t('preferences.save') }}
        </el-button>
        <el-button @click="resetForm('advancedForm')">
...
export default {
  ...
  methods: {
    onCheckUpdateClick() {
      ...
      this.$store.dispatch('preference/fetchPreference').then(config => {
    ...
    syncTrackerFromSource() {
      ...
      this.$store
        .dispatch('preference/fetchBtTracker', trackerSource)
        .then(data => {
      ...
      dialog
        ...
        .then(({ response }) => {
          if (response === 0) {
            ...
            this.$store.dispatch('task/pauseAllTask').then(() => {
    ...
    syncFormConfig() {
      this.$store.dispatch('preference/fetchPreference').then(config => {
        ...
        if (changedConfig.basic.theme !== undefined) {
          this.$electron.ipcRenderer.send('command', 'application:change-theme', changedConfig.basic.theme)
        }
      })
    },
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        ...
        const { btAutoDownloadContent, autoHideWindow, btTracker, noProxy, rpcListenPort } = data
        ...
        if (this.isRenderer) {
          if ('autoHideWindow' in data) {
            this.$electron.ipcRenderer.send('command', 'application:auto-hide-window', autoHideWindow)
          }
      ...
      if (isEmpty(changedConfig.basic) && isEmpty(changedConfig.advanced)) {
        ...
        dialog
          ...
          .then(({ response }) => {
            if (response === 0) {
              if (changedConfig.basic.theme !== undefined) {
                this.$electron.ipcRenderer.send('command', 'application:change-theme', backupConfig.theme)
              }
